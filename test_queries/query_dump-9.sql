		...
		WITH offers_data AS (
			...
			SELECT
				...
				-- joins expects that
				'c8b3b059-a5f3-4c15-94a3-413a04713c9e' AS offer_id,
				NULL AS updated_at,
				NULL AS product_id,
				'kelkoo_NO' AS offer_source,
				'Steiner Navigator Pro 7x50 Med Kompass Marinekikkert Med Innebygget Kompass' AS retail_prod_name,
				'Foto.no' AS retailer_name,
				'NO' AS country,
				'679500' AS price,
				'NOK' AS currency,
				'https://no-go.kelkoogroup.net/ctl/go/sitesearchGo?.ts=1605865837001&.sig=bmNYP666F5iyAcTXZEcwXVp46Jc-&affiliationId=96954748&catId=164401&comId=16109413&contextLevel=1&contextOfferPosition=1&contextPageSize=1&country=no&ecs=ok&merchantid=16109413&offerId=43b61aa630fce6f11b94d23dfb266a6a&searchId=1076992064468_1605865836994_29553&searchQuery=&service=5&wait=true' AS offer_url,
				'2020-11-20 09:50:37.009890' AS requested_at,
			...
			SELECT
				-- We need to create a random ID since the existing
				-- joins expects that
				'4f23cd95-11c9-492d-93c4-daf6a2bea611' AS offer_id,
				NULL AS updated_at,
				NULL AS product_id,
				'kelkoo_SE' AS offer_source,
				'Steiner Navigator Pro 7x50 Compass' AS retail_prod_name,
				'CyberPhoto' AS retailer_name,
				'SE' AS country,
				'599000' AS price,
				'SEK' AS currency,
				'https://se-go.kelkoogroup.net/ctl/go/sitesearchGo?.ts=1605865837072&.sig=GgSpfvDdrIth5wiguYs4P9d4qxk-&affiliationId=96954747&catId=164401&comId=3375501&contextLevel=1&contextOfferPosition=1&contextPageSize=2&country=se&ecs=ok&merchantid=3375501&offerId=2bc5fbfe700d4146562cf20ba3b9692a&searchId=10769825018469_1605865837064_40800&searchQuery=&service=5&wait=true' AS offer_url,
				'2020-11-20 09:50:37.082095' AS requested_at,
				1::int AS match_score
			UNION ALL
			...
			SELECT
				-- We need to create a random ID since the existing
				-- joins expects that
				'b1c53d76-a2f1-4773-bd51-69c6fc3402f0' AS offer_id,
				NULL AS updated_at,
				NULL AS product_id,
				'kelkoo_SE' AS offer_source,
				'Steiner 7x50 Navigator Pro Compass' AS retail_prod_name,
				'Gofoto.se' AS retailer_name,
				'SE' AS country,
				'649000' AS price,
				'SEK' AS currency,
				'https://se-go.kelkoogroup.net/ctl/go/sitesearchGo?.ts=1605865837072&.sig=xLgFRjhvj4bDfNdDJY02oaTIJLg-&affiliationId=96954747&catId=164401&comId=15655613&contextLevel=1&contextOfferPosition=2&contextPageSize=2&country=se&ecs=ok&merchantid=15655613&offerId=89a03f43e4973575bdaaf988de4495c3&searchId=10769825018469_1605865837064_40800&searchQuery=&service=5&wait=true' AS offer_url,
				'2020-11-20 09:50:37.082124' AS requested_at,
				1::int AS match_score
			UNION ALL
