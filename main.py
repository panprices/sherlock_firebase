...
def _initialize_firebase():
    # Use Application Default Credentials or GOOGLE_APPLICATION_CREDENTIALS env var
...
def product_search_trigger(event, context, production=True):
    ...
    if (
        production
        and ("results" not in event["delta"] or len(event["delta"]["results"]) == 0)
        and "name" in event["delta"]
        and "path_name" in event["delta"]
    ):
        try:
            end = time.time()
            ...
            product_search_trigger_publisher = Publisher(
                "panprices", "sherlock_google_shopping"
            )
            pub_results = product_search_trigger_publisher.publish_messages(
                [event["delta"]]
            )
        except Exception as e:
            raise e
...
def product_search_publish_result(event, context, production=True):
    ...
    try:
        ...
        product_token = encryption.fernet_encrypt(payload["gtin"].split(", ")[0])
        ...
        if production:
            ...
            if "performance" in payload:
                ...
                result[product_token]["performance"][
                    "product_search_publish_result"
                ] = {
                    "start": start * 1000,
                    "end": end * 1000,
                    "exeTime": (end - start) * 1000,
                    "delay_from_sherlock_upload_image": start * 1000
                    - payload["performance"]["sherlock_upload_image"]["end"],
                }
            current_entry_ref.update({"results": result})
